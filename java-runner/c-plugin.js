...
app.post('/api/c/prepare', async (req, res) => {
  try {
    ...
    // newline-based bash script (robust; no glued && after ;)
...
files=(./**/*.c ./*.c)
if (( \${#files[@]} == 0 )); then echo 'No .c files found'; exit 1; fi
printf '$ gcc -std=c17 -O2 -pipe -Wall -Wextra -Wno-unused-result -o main'
for f in "\${files[@]}"; do printf ' %q' "\${f}"; done
printf ' -lm\\n'
gcc -std=c17 -O2 -pipe -Wall -Wextra -Wno-unused-result -o main "\${files[@]}" -lm
`;
    ...
    proc.on('close', (code) => {
      ...
      const token = uid();
      // If you don't have libstdin_notify.so, you can drop LD_PRELOAD entirely:
...
server.on('upgrade', (req, socket, head) => {
  try {
    ...
    if (pathname !== '/term-c') return;          // << must be /term-c
